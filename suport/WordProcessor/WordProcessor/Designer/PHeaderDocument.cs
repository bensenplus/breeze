...
{
    public class PHeaderDocument : MDocument
    {
        public DPageHeader OwnerPheader;

        public PHeaderDocument(DPageHeader pageHeader)
        {
            OwnerPheader = pageHeader;
            CreateRow();
        }
        ...
        {
            if (FirstRow == null || OwnerPheader == null) return;
            FirstRow.X = OwnerPheader.X + OwnerPheader.PaddingLeft;
            FirstRow.Y = OwnerPheader.Y + OwnerPheader.PaddingTop;
            FirstRow.Height = EditorSetting.DefaultDocRowHeight;
            FirstRow.Width = OwnerPheader.Width - OwnerPheader.PaddingLeft - OwnerPheader.PaddingRight;
        }
        ...
        {
            var tempHeader = OwnerPheader.GetTopPageHeader();
            while (tempHeader != null)
            {
                if (tempHeader.Equals(OwnerPheader))
                {
                    tempHeader = tempHeader.NextPageHeader;
                    continue;
                }
                SynchronizeData(tempHeader);
                tempHeader = tempHeader.NextPageHeader;
            }
        }

        public void SynchronizeData(DPageHeader pageHeader)
        {
            if (pageHeader == null) return;
            var doc = pageHeader.OwnerDocument;
            doc.FirstRow = null;
            doc.LastRow = null;
            var data = CopyData();
        ...
        {
            return OwnerPheader.X + OwnerPheader.PaddingLeft;
        }

        public override int GetPageBodyWidth()
        {
            return OwnerPheader.Width;
        }
    }
}
