...
namespace DbAccess
{
    ...
    public class OracleToSQLite
    {
        ...
        /// </summary>
        /// <param name="oracleConnString">The connection string to the SQL Server database.</param>
        ...
        /// <param name="handler">A handler to handle progress notifications.</param>
        private static void CopyOracleRowsToSQLiteDB(
            ...
            string password, SqlConversionHandler handler)
        {
            ...
            using (OracleConnection ssconn = new OracleConnection(sqlConnString))
            {
                ssconn.Open();
 ...
 //                       try
                        {
                            string tableQuery = BuildOracleTableQuery(schema[i]);
                            OracleCommand query = new OracleCommand(tableQuery, ssconn);
...
//                        } // catch
        ...
        private static object CastValueForColumn(object val, ColumnSchema columnSchema)
        {
            ...
            //DbType dt = GetDbTypeByOracleType(columnSchema);
            DbType dt = columnSchema.ColumnType;
        ...
        private static SQLiteCommand BuildSQLiteInsert(TableSchema ts)
        {
            ...
            for (int i = 0; i < ts.Columns.Count; i++)
            {
                ...
                DbType dbType = ts.Columns[i].ColumnType;
        ...
        private static DbType GetDbTypeByOracleType(String columnType)
        {
            DbType dbType;
            switch (columnType)
            {
                ...
                case "BLOB":
                    ...
                    break;
                default:
                    throw new ApplicationException("Illegal DB type found (" + columnType + ")");
            }
            return dbType;
        }

        private static string GetSQLiteTypeByDbType(DbType dbType)
        {
            String sqliteType;
            switch (dbType)
            {
                ...
                case DbType.Binary:
                    ...
                    break;
                default:
                    throw new ApplicationException("Illegal DB type found (" + dbType + ")");
            }
            return sqliteType;
        }
        ...
        private static string BuildOracleTableQuery(TableSchema ts)
        {
            StringBuilder sb = new StringBuilder();
            sb.Append("SELECT ");
            for (int i = 0; i < ts.Columns.Count; i++)
            {
                sb.Append(ts.Columns[i].ColumnName);
                if (i < ts.Columns.Count - 1) sb.Append(", ");
            } // for
            sb.Append(" FROM " + ts.TableName);// + " where rownum <1000 ");
            return sb.ToString();
        }
        ...
        private static string BuildColumnStatement(ColumnSchema col, TableSchema ts, ref bool pkey)
        {
            ...
            if (col.IsIdentity)
            {
                if (ts.PrimaryKey.Count == 1 && (col.ColumnType == DbType.Byte || col.ColumnType == DbType.Int32 || col.ColumnType == DbType.Int16 ||
                    col.ColumnType == DbType.Int64))
                {
            ...
            {
                sb.Append(GetSQLiteTypeByDbType(col.ColumnType));
...
//                dataType == "NUMBER" || dataType == "TIMESTAMP(6)" ||
//                dataType == "DATE" || dataType == "nvarchar" ||
...
//        }
        ...
        private static void CreateForeignKeySchema(OracleConnection conn, TableSchema ts)
        {
        ...
        private static ILog _log = LogManager.GetLogger(typeof(OracleToSQLite));
        #endregion
    }
