        ...
        private static object ReadValue (Type inst_type, JsonReader reader)
        {
            ...
            if (reader.Token == JsonToken.Double ||
                ...
                if (inst_type == typeof(DateTime))
                {
                    ...
                    return new DateTime(time * 10000 + 621355968000000000, DateTimeKind.Utc).ToLocalTime();                       
                }
                                        ...
                                        int depth)
        {
            ...
            foreach (PropertyMetadata p_data in props) {
                if (p_data.IsField) {
                    ...
                    if (p_info.CanRead) {
                        writer.WritePropertyName (p_data.Info.Name);
                        if (p_info.PropertyType == typeof(DateTime))
                        {   
                            DateTime value = (DateTime)p_info.GetValue(obj, null);
                            WriteValue(value.ToString("yyyy-MM-dd'T'HH:mm:ss.fff'Z'"), writer, writer_is_private, depth + 1);
                        }
                        else
                        {
